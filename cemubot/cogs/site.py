...
class Site(commands.Cog):
    ...
    async def update_activity(self):
        if datetime.utcnow() < self.public_release_date:
            if self.version_list[0] == self.patreon_release:
                new_activity = f"Cemu {self.patreon_release} is publicly released!"
            else:
    ...
    async def on_message(self, message):
        if message.channel.id == config.cfg["announcement_channel"]:
